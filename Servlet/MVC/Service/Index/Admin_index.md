...
1. 여러 개의 공지에 대한 일괄 공개 : pubNoticeAll(ids)
2. 여러 개의 공지에 대한 일괄 삭제 : removeNoticeAll(ids)
3. 공지 글쓰기 작성
   - 공지 등록 요청(insertNotice(notice))
   - 공지 삭제 요청(deleteNotice(id))
   - 공지 수정 요청(updateNotice(notice))
4. 페이지 요청 : getNoticeNewestList()
